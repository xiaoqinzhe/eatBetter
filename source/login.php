...
if(!isset($_GET['type']))
{	
	echo getJsonResponse(2,'type参数没有设置',null);
	exit;
}
$sql='';
$sql1='';
...
if($_GET['type']=='username'){
	if(isset($_POST['username'])&&isset($_POST['password'])){
		...
		$sql1="select user_id,access_token from users where username='{$_POST['username']}';";
	}else{
	...
	if(isset($_POST['phone'])&&isset($_POST['password'])){
		...
		$sql1="select user_id,access_token from users where phone='{$_POST['phone']}';";		
	}else{
		echo getJsonResponse(2,'post参数没有设置',null);
	}
}else{
	echo getJsonResponse(2,'type参数错误',null);
}
//查询数据库，验证密码
$db=Db::getInstance();
try {
	...
	if(!$result){
		...
		if(md5($_POST['password'])==$result[0]['password'])   
		{
			...
			if($res){
				...
				echo getJsonResponse(2,"查询id出错",$data);
			}
		}
		else{
